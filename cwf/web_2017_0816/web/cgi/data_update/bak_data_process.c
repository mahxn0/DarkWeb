...
int display_param_process1(entry_buf in_buf[],int cont_value)
{
	...
	sprintf(&value[0], "%s://%s:%s@%s:%s/%s/%s/%s/%s",
		in_buf[7].value,in_buf[3].value,in_buf[4].value,in_buf[5].value,
		in_buf[8].value,in_buf[9].value,in_buf[6].value,in_buf[2].value,"av_stream");
	for(i=0; i<4; i++)
	{
		sprintf(&value[i], "%s://%s:%s@%s:%s/%s/%s/%s/%s",
		in_buf[7+11*i].value,in_buf[3+11*i].value,in_buf[4+11*i].value,in_buf[5+11*i].value,
		in_buf[8+11*i].value,in_buf[9+11*i].value,in_buf[6+11*i].value,in_buf[2+11*i].value,"av_stream");
		//printf("%s\n",&value[i]);
		...
		save_to_xml(buff,in_buf[0+11*i].value);
...
int  get_index(entry_buf in_buf[])
{
	...
	else if(strcmp(in_buf[0].name,"camera1_setting_switch")==0)
	{
		in_buf[0].index=56;
		ret=1;
	}
