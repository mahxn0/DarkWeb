...
void offline_pic()
{
	...
	memset(value, 0 ,sizeof(value));
	get_from_xml("offline.load_flag",value);
	load_flag = atoi(value);

	memset(value, 0 ,sizeof(value));
	get_from_xml("offline.det_sig",value);
	det_sig = atoi(value);
	if(det_sig == 1)
	{
		save_to_xml("offline.det_sig", "0");
	}

	get_from_xml("offline.pic_url",pic_url);

	get_from_xml("offline.type",type);
