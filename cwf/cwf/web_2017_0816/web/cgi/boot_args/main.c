...
void ip_addr()
{
	...
	printf("%s\n",inet_ntoa(*(struct in_addr *)&decoder_param.camera_net.gateway));
		...
		get_from_xml("server.port",value);
		decoder_param.serverport = atoi(value);
...
#if 1
		get_from_xml("server.ip",decoder_param.serverip);
		//get_from_xml("server.netmask",decoder_param.serverport);  //cwf
		memset(value, 0 ,sizeof(value));
		//get_from_xml("server.port",decoder_param.serverport);
		get_from_xml("server.port",value);
		decoder_param.serverport = atoi(value);
...
#else
	char enable[8][32] = {0};
	char jc_enable[8][32] = {0};
	char rstp[8][320] = {0};
	int i , n;
	char setting_total[32];
	int num;
	char buff[128];

	
	get_from_xml("rtsp.setting_total",setting_total);
	num = atoi(setting_total);

	for(i = 0; i < num; i++)
	{
		memset(buff, 0 , sizeof(buff));
		sprintf(buff,"rtsp.enable%d",i+1);
		get_from_xml(buff,enable[i]);
	}
