...
int display_param_process1(entry_buf in_buf[],int cont_value)
	{
...
#if 0
		printf("0--%s = %s\n",in_buf[0].name,in_buf[0].value);	//相机开关
		printf("1--%s = %s\n",in_buf[1].name,in_buf[1].value);	//检测开关
		printf("3--%s = %s\n",in_buf[2].name,in_buf[2].value);	//码流类型
		printf("5--%s = %s\n",in_buf[3].name,in_buf[3].value);	//用户名
		printf("6--%s = %s\n",in_buf[4].name,in_buf[4].value);	// 密码
		printf("7--%s = %s\n",in_buf[5].name,in_buf[5].value);	//ip地址
		printf("2--%s = %s\n",in_buf[6].name,in_buf[6].value);	//通道号
		printf("4--%s = %s\n",in_buf[7].name,in_buf[7].value);	// 传输协议
		printf("8--%s = %s\n",in_buf[8].name,in_buf[8].value);	//端口号
		printf("9--%s = %s\n",in_buf[9].name,in_buf[9].value);	//视频编码
		printf("10--%s = %s\n",in_buf[10].name,in_buf[10].value);  //none
#endif
	
#if 1
		 //拼接如下：rtsp://admin:buaa123456@192.168.1.106:554/h264/ch1/main/av_stream
		//printf("0--%s = %s\n",in_buf[0].name,in_buf[0].value);
		save_to_xml("rtsp.setting_total",in_buf[0].value);
		if(strcmp(in_buf[1].name, "toggle_camera") == 0)
		{
			save_to_xml("rtsp.toggle_camera",in_buf[1].value);
			num = atoi(in_buf[0].value);
			memset(value, 0 ,sizeof(value));
		#if 0
			memset(value, 0 ,sizeof(value));
				sprintf(&value[0], "%s://%s:%s@%s:%s/%s/%s/%s/%s",
				in_buf[8+11*5].value,in_buf[4+11*5].value,in_buf[5+11*5].value,in_buf[6+11*5].value,
				in_buf[9+11*5].value,in_buf[10+11*5].value,in_buf[7+11*5].value,in_buf[3+11*5].value,"av_stream");
			printf("value[5] = %s\n",value[5]);
		#endif
			for(i=0; i<num; i++)  //待该	
			{
				memset(value, 0 ,sizeof(value));
			#if 0
				sprintf(&value[i], "%s://%s:%s@%s:%s/%s/%s/%s/%s",
				in_buf[9+11*i].value,in_buf[5+11*i].value,in_buf[6+11*i].value,in_buf[7+11*i].value,
				in_buf[10+11*i].value,in_buf[11+11*i].value,in_buf[8+11*i].value,in_buf[4+11*i].value,"av_stream");
			#else
				sprintf(&value[i], "%s://%s:%s@%s:%s/%s/%s/%s/%s",
				in_buf[9+11*i].value,in_buf[6+11*i].value,in_buf[7+11*i].value,in_buf[8+11*i].value,
				in_buf[10+11*i].value,in_buf[11+11*i].value,in_buf[5+11*i].value,in_buf[4+11*i].value,"av_stream");
			#endif
				//printf("%s\n",&value[i]);
	
				memset(buff, 0 ,sizeof(buff));
				sprintf(buff, "rtsp.rtsp_url_%d", i+1);
				//printf("1-----%s\n",buff);
				save_to_xml(buff,value[i]);
	
				memset(buff, 0 ,sizeof(buff));
				sprintf(buff, "rtsp.enable%d", i+1);
				//printf("%d-----%s\n",i ,in_buf[0+11*i].value);
				save_to_xml(buff,in_buf[2+11*i].value);
	
				memset(buff, 0 ,sizeof(buff));
				sprintf(buff, "rtsp.jc_enable%d", i+1);
				//printf("%d-----%s\n", i, in_buf[1+11*i].value);
				save_to_xml(buff,in_buf[3+11*i].value);
			}
	
		}
		else
		{
			save_to_xml("rtsp.toggle_camera", "off");
			num = atoi(in_buf[0].value);
			memset(value, 0 ,sizeof(value));
			...
			for(i=0; i<num; i++)  //待该	
			{
			...
			#else
				sprintf(&value[i], "%s://%s:%s@%s:%s/%s/%s/%s/%s",
				in_buf[8+11*i].value,in_buf[5+11*i].value,in_buf[6+11*i].value,in_buf[7+11*i].value,
				in_buf[9+11*i].value,in_buf[10+11*i].value,in_buf[4+11*i].value,in_buf[3+11*i].value,"av_stream");
			#endif
				//printf("%s\n",&value[i]);
				...
				sprintf(buff, "rtsp.enable%d", i+1);
				//printf("%d-----%s\n",i ,in_buf[0+11*i].value);
				save_to_xml(buff,in_buf[1+11*i].value);
...
int display_param_process2(entry_buf in_buf[],int cont_value)
{
	...
	save_to_xml("rtsp.roll_time",in_buf[0].value);
...
int  get_index(entry_buf in_buf[])
{
	...
	else if(strcmp(in_buf[0].name,"setting_total")==0)
	{
		in_buf[0].index=56;
		ret=1;
	}
	else if(strcmp(in_buf[0].name,"camera_polling_name")==0)
	{
		in_buf[0].index=57;
		ret=1;
	}
